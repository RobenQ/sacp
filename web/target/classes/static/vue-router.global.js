          ...
          for (const name in record.components) {
              let rawComponent = record.components[name];
              {
                  if (!rawComponent ||
                      ...
                      warn(`Component "web" in record with path "${record.path}" is not` +
                      ...
                      warn(`Component "web" in record with path "${record.path}" is a ` +
                      ...
                      warn(`Component "web" in record with path "${record.path}" is defined ` +
              ...
              if (isRouteComponent(rawComponent)) {
                  ...
                  if (!('catch' in componentPromise)) {
                      warn(`Component "web" in record with path "${record.path}" is a function that does not return a Promise. If you were passing a functional component, make sure to add a "displayName" to the component. This will break in production if not fixed.`);
                  ...
                  guards.push(() => componentPromise.then(resolved => {
                      if (!resolved)
                          return Promise.reject(new Error(`Couldn't resolve component "web" at "${record.path}"`));
